...
def make_fn(model_class, process_name, counter):
    ...
    def _function(*args, **kwargs):
        ...
        if model_class.to_batch and not viper_config.multiprocessing:
    ...
    if mp.current_process().name == 'MainProcess':
        ...
        for model_class_ in list_models:
            for process_name_ in model_class_.list_processes():
                if process_name_ in viper_config.load_models and viper_config.load_models[process_name_]:
...
def forward(model_name, *args, queues=None, **kwargs):
    ...
    if not viper_config.multiprocessing:
        try:
