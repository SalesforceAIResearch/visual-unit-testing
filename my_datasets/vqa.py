...
class VQADataset(BaseDataset):
    def __init__(self, 
        ...
        if sample_per_type is not None and sample_per_type>0:
            ...
            samples_dir = './my_datasets/samples/VQA'
            ...
            if os.path.exists(sample_per_type_file):
                ...
                id2type = {d['sample_id']: d['reasoning_answer_most_common'] for d in tqdm(self.data)}
                ...
                self.data = [d for d in self.data \
                    if d['sample_id'] in type2id[d['reasoning_answer_most_common']]]
                pickle.dump(self.data, open(sample_per_type_file, 'wb'))
        
        for d in self.data:
            d['image'] = d['image_id']
            d['answer']= d['reasoning_answer_most_common']
            d['question'] = d['reasoning_question']
