...
def load_image(path):
    if path.startswith("http://") or path.startswith("https://"):
        image = Image.open(requests.get(path, stream=True).content).convert('RGB')
...
def get_code(query):
    model_name_codex = 'codellama' if viper_config.codex.model == 'codellama' else 'codex'
    code = forward(model_name_codex, prompt=query, input_type="image")
    if viper_config.codex.model not in ('gpt-3.5-turbo', 'gpt-4'):
...
###### HTML OUTPUT ######
def show_all_html(lineno, value, valuename, fig=None, usefig=True, disp=True, console_in=None, time_wait_between_lines=None, lastlineno=[-1]):
    time.sleep(0.1)  # to avoid race condition!
    ...
    thing_to_show = value
    if lineno is not None and lineno != lastlineno[0]:
        html_output += f"<hr><h3>Line {lineno}</h3>"
        lastlineno[0] = lineno  # ugly hack

    # if usefig and fig is not None:
    #     plt.clf()
    #     ax = fig.add_axes([0, 0, 1, 1])
    #     ax.set_xticks([])
    #     ax.set_yticks([])
    ...
    if isinstance(thing_to_show, Image.Image):
        if valuename:
            html_output += f"<p>{rich_escape(valuename)} = </p>"
        img_html = f'<img src="{image_to_base64(thing_to_show)}" alt="{valuename}" style="max-width:400px;max-height:400px;">'
        html_output += f"<div>{img_html}</div>"

    elif str(type(thing_to_show)) == "<class 'image_patch.ImagePatch'>":
        if valuename:
            html_output += f"<p>{rich_escape(valuename)} = </p>"
        img_html = f'<img src="{image_to_base64(thing_to_show.cropped_image)}" alt="{valuename}" style="max-width:400px;max-height:400px;">'
        html_output += f"<div>{img_html}</div>"

    elif isinstance(thing_to_show, (list, tuple)):
        if len(thing_to_show) > 0:
            html_output += "<ul>"
            for i, thing in enumerate(thing_to_show):
                disp_ = disp or i < len(thing_to_show) - 1
                item_html = show_all_html(None, thing, f"{rich_escape(valuename)}[{i}]", fig=fig, disp=disp_, usefig=usefig)
                html_output += f"<li>{item_html}</li>"
            html_output += "</ul>"
        else:
            html_output += f"<p>{rich_escape(valuename)} is empty</p>"
        ...
        if len(thing_to_show) > 0:
            ...
            for i, (thing_k, thing_v) in enumerate(thing_to_show.items()):
                disp_ = disp or i < len(thing_to_show) - 1
                item_html = show_all_html(None, thing_v, f"{rich_escape(valuename)}['{thing_k}']", fig=fig, disp=disp_, usefig=usefig)
            html_output += f"<li>{thing_k}: {item_html}</li>"
            html_output += "</ul>"
        else:
            html_output += f"<p>{rich_escape(valuename)} is empty</p>"

    else:
        html_output += f"<p>{rich_escape(valuename)} = {thing_to_show}</p>"
        if time_wait_between_lines is not None:
            time.sleep(time_wait_between_lines / 2)

    # Display the final figure if needed
    # if usefig and fig is not None:
    #     fig.set_size_inches(2, 2)
    #     img_buf = BytesIO()
    ...
    #     img_html = f'<img src="data:image/png;base64,{img_str}" alt="{valuename}" style="max-width:400px;max-height:400px;">'
    #     html_output += f"<div>{img_html}</div>"
    
    print(html_output)
            

def inject_saver_html(code, show_intermediate_steps, syntax=None, time_wait_between_lines=None, console=None):
    injected_function_name = 'show_all_html'
    if injected_function_name in code:
        return code
    code = code.split("\n")
    newcode = []
...
def execute_code_html(code, im, show_intermediate_steps=True, with_result=False):

    code, syntax = code
    html_output = "<h2>Code Execution</h2>"
    html_output += f"<pre>{rich_escape(code)}</pre>"
    code_line = inject_saver_html(code, show_intermediate_steps, syntax, time_wait_between_lines, console)
    ...
    try:
        ...
        with contextlib.redirect_stdout(terminal_output):
            result = execute_command(im, my_fig, time_wait_between_lines, syntax)
        output_str = terminal_output.getvalue()
        terminal_output.close()
        html_output += f"<h2>Output</h2>"
        html_output += f"<pre>{rich_escape(output_str)}</pre>"
        # Convert final result to HTML if it's an image
        if isinstance(result, Image.Image):
            img_html = f'<img src="{image_to_base64(result)}" alt="result image" style="max-width:400px;max-height:400px;">'
            html_output += f"<div>{img_html}</div>"

        # Handle other possible types of output (e.g., string, dict, list, etc.)
            ...
            for item in result:
                if isinstance(item, Image.Image):
                    img_html = f'<img src="{image_to_base64(item)}" alt="result image" style="max-width:400px;max-height:400px;">'
                    html_output += f"<li>{img_html}</li>"
                else:
            ...
            for key, value in result.items():
                if isinstance(value, Image.Image):
                    img_html = f'<img src="{image_to_base64(value)}" alt="result image" style="max-width:400px;max-height:400px;">'
                    html_output += f"<li>{key}: {img_html}</li>"
                else:
        ...
        html_output += f"<p style='color:red;'>Encountered error {e} when trying to run the code.</p>"
    if with_result:
