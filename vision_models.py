"""
From: https://github.com/cvlab-columbia/viper
...
from viper_utils import HiddenPrints
...
HiddenPrints = partial(HiddenPrints, console=console, use_newline=viper_config.multiprocessing)
...
class CLIPModel(BaseModel):
    ...
    def get_clip_transforms_from_PIL(self, n_px=336):
        return transforms.Compose([
            # transforms.ToPILImage(),
            transforms.Resize(n_px, interpolation=transforms.InterpolationMode.BICUBIC),
            transforms.CenterCrop(n_px),
    ...
    @torch.no_grad()
    def classify(self, image: Union[torch.Tensor, list], categories, return_index=True):
    ...
    @torch.no_grad()
    def compare(self, images, prompt, return_scores=False):
...
class MaskRCNNModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, threshold=viper_config.detect_thresholds.maskrcnn):
        super().__init__(gpu_number)
...
class OwlViTModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, threshold=viper_config.detect_thresholds.owlvit):
        super().__init__(gpu_number)
...
class TCLModel(BaseModel):
    ...
    @torch.no_grad()
    def binary_score(self, images, prompt):
        single_image = False
...
class GPT3Model(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        with open(viper_config.gpt3.qa_prompt) as f:
            self.qa_prompt = f.read().strip()
        with open(viper_config.gpt3.guess_prompt) as f:
            self.guess_prompt = f.read().strip()
        self.temperature = viper_config.gpt3.temperature
        self.n_votes = viper_config.gpt3.n_votes
        self.model = viper_config.gpt3.model
    ...
    def get_qa(self, prompts, prompt_base: str = None):
...
class LLMModel(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        from transformers import pipeline, BitsAndBytesConfig, LlamaForCausalLM, AutoTokenizer

        # Load Llama2
        model_id = viper_config.llm.model_id
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_id, token=os.getenv('HF_ACCESS_TOKEN'))
    ...
    def process_prompts(self, prompts,  **kwargs):
        responses = []
        for prompt in prompts:
            generated = self.generator(prompt, **kwargs)
            if kwargs.get('num_return_sequences', 1) == 1:
                responses.append(generated[0]['generated_text'].strip())
            else:
                responses.extend([g['generated_text'].strip() for g in generated])
        return responses

    def get_qa(self, prompts,  **kwargs):
        return self.process_prompts(prompts, **kwargs)

    def get_guesses(self, prompts, **kwargs):
        return self.process_prompts(prompts, **kwargs)

    def get_general(self, prompts, **kwargs):
        return self.process_prompts(prompts,  **kwargs)

    def forward(self, prompt, process_name, **kwargs):
        process_map = {
            'llm_qa': self.get_qa,
            'llm_guess': self.get_guesses,
            'llm_general': self.get_general
        }
        results = process_map[process_name](prompt, **kwargs)
        if not self.to_batch:
            results = results[0]
        return results
    ...
    @classmethod
    def list_processes(cls):
        return ['llm_' + n for n in ['qa', 'guess', 'general']]

# @cache.cache
@backoff.on_exception(backoff.expo, Exception, max_tries=10)
def codex_helper(extended_prompt):
    assert 0 <= viper_config.codex.temperature <= 1
    assert 1 <= viper_config.codex.best_of <= 20

    if viper_config.codex.model in ("gpt-4", "gpt-3.5-turbo"):
        if not isinstance(extended_prompt, list):
            extended_prompt = [extended_prompt]
        responses = [openai.ChatCompletion.create(
            model=viper_config.codex.model,
            messages=[
                # {"role": "system", "content": "You are a helpful assistant."},
                {"role": "system", "content": "Only answer with a function starting def execute_command."},
                {"role": "user", "content": prompt}
            ],
            temperature=viper_config.codex.temperature,
            max_tokens=viper_config.codex.max_tokens,
            top_p=1.,
            frequency_penalty=0,
            presence_penalty=0,
    ...
    #             resp = resp[0]
        ...
        response = openai.Completion.create(
            ...
            frequency_penalty=0,
            presence_penalty=0,
            best_of=viper_config.codex.best_of,
            stop=["\n\n"],
        )
...
class CodexModel(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        super().__init__(gpu_number=gpu_number)
        with open(viper_config.codex.prompt) as f:
            self.base_prompt = f.read().strip()
        self.fixed_code = None
        if viper_config.use_fixed_code:
            with open(viper_config.fixed_code_file) as f:
                self.fixed_code = f.read()

    def forward(self, prompt, input_type='image', prompt_file=None, base_prompt=None, extra_context=None, **kwargs):
        if viper_config.use_fixed_code:  # Use the same program for every sample, like in socratic models
        ...
        if extra_context is None:
            extra_context = [''] * len(prompt) if isinstance(prompt, list) else ''
        ...
        result = self.forward_(extended_prompt, **kwargs)
        if not isinstance(prompt, list):
            result = result[0]

        return result

    def forward_(self, extended_prompt, **kwargs):
        ...
        try:
            ...
            if len(sub_batch_1) > 0:
                response_1 = self.forward_(sub_batch_1, **kwargs)
            else:
                response_1 = []
            if len(sub_batch_2) > 0:
                response_2 = self.forward_(sub_batch_2, **kwargs)
            else:
                response_2 = []
            ...
            print(e)
            response = self.forward_(extended_prompt, **kwargs)
        return response
...
class CodeLlama(CodexModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        # Load Llama2
        model_id = viper_config.codex.codellama_model_name
        ...
        self.tokenizer = CodeLlamaTokenizer.from_pretrained(model_id,  token=os.getenv('HF_ACCESS_TOKEN'), trust_remote_code=True)
    ...
    def run_codellama(self, prompt, **kwargs):
        input_ids = self.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True)["input_ids"]
        generated_ids = self.model.generate(input_ids.to("cuda"), **kwargs)
    ...
    def forward_(self, extended_prompt, **kwargs):
        if len(extended_prompt) > self.max_batch_size:
            response = []
            for i in range(0, len(extended_prompt), self.max_batch_size):
                response += self.forward_(extended_prompt[i:i + self.max_batch_size], **kwargs)
            return response
        with torch.no_grad():
            response = self.run_codellama(extended_prompt, **kwargs)
        # Clear GPU memory
        torch.cuda.empty_cache()
        return response

class InterLMModel(BaseModel):
    name = 'internlm'
    to_batch = True
    max_batch_size = 32
    seconds_collect_data = 0.2  # The queue has additionally the time it is executing the previous forward pass

    def __init__(self, gpu_number=0, half_precision=viper_config.blip_half_precision,
                ):
        super().__init__(gpu_number)
        ...
        with warnings.catch_warnings(), HiddenPrints("INTERLM"), torch.cuda.device(self.dev):
            max_memory = {gpu_number: torch.cuda.mem_get_info(self.dev)[0]}

            self.processor = AutoTokenizer.from_pretrained('internlm/internlm-xcomposer2d5-7b',  token=os.getenv('HF_ACCESS_TOKEN'), trust_remote_code=True)
            ...
            try:
                ...
                self.model = AutoModelForCausalLM.from_pretrained(
                   'internlm/internlm-xcomposer2d5-7b', #quantization_config=bnb_config if half_precision else None,
                    torch_dtype=torch.bfloat16 if half_precision else torch.float32,
                     max_memory=max_memory,
                    trust_remote_code=True,
                    token=os.getenv('HF_ACCESS_TOKEN'),
                    assign=True
    ...
    @torch.no_grad()
    def caption(self, image, prompt="Analyze the given image in a detail manner"):
        print(image)
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            response, _ = self.model.chat(self.processor, prompt,image, do_sample=False,num_beams=3, use_meta=True)
        return response
    ...
    def qa(self, image, question):
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            response, _ = self.model.chat(self.processor, question, image, do_sample=False,num_beams=3, use_meta=True)
        return response
...
class GroundingDinoHFModel(BaseModel):
    name = 'dino_hf'
    to_batch = True
    max_batch_size = 2
    seconds_collect_data = 0.2  # The queue has additionally the time it is executing the previous forward pass

    def __init__(self, gpu_number=0, box_threshold=viper_config.dino.box_threshold,
                 text_threshold=viper_config.dino.box_threshold, model_size=viper_config.dino.model_size):
        super().__init__(gpu_number)
        ...
        assert model_size in ['base', 'tiny']
        with warnings.catch_warnings(), HiddenPrints("DINOHF"), torch.cuda.device(self.dev):
            max_memory = {gpu_number: torch.cuda.mem_get_info(self.dev)[0]}
            model_id = f"IDEA-Research/grounding-dino-{model_size}"
            self.processor = AutoProcessor.from_pretrained(model_id, token=os.getenv('HF_ACCESS_TOKEN'))
            # Device_map must be sequential for manual GPU selection
            try:
    ...
    @torch.no_grad()
    def compute_prediction(self, image, obj, return_labels=False, confidence_threshold=viper_config.dino.box_threshold, text_threshold=viper_config.dino.text_threshold):
        if isinstance(obj, str):
            obj = [obj]
        if not isinstance(image, list):
            image = [self.toPIL(image)]
        else:
            image = [self.toPIL(im) for im in image]
        def preprocess_caption(caption: str) -> str:
            result = caption.lower().strip()
            if result.endswith("."):
                return result
            return result + "."
        obj = [preprocess_caption(o) for o in obj]
        widths = [im.size[0] for im in image]
        heights = [im.size[1] for im in image]  
        inputs = self.processor(images=image, text=obj, padding=True, return_tensors="pt").to(self.dev)
        with torch.cuda.device(self.dev):
            outputs = self.model(**inputs)
        ...
        scores, phrases, bboxes_xyxy = [r['scores'] for r in results], [r['labels'] for r in results], [r['boxes'] for r in results]
        boxes= []
        for i, (h,w) in enumerate(zip(heights, widths)):
            left, lower, right, upper = bboxes_xyxy[i][:, 0], bboxes_xyxy[i][:, 1], bboxes_xyxy[i][:, 2], bboxes_xyxy[i][:, 3]
            ...
            boxes.append(torch.stack([left, lower, right, upper], -1).detach().cpu().numpy())
        if return_labels:
            return boxes[:10], phrases[:10]
        return boxes[:10]
   

    def forward(self, image, obj, **kwargs):
        response = self.compute_prediction(image=image, obj=obj, **kwargs)
        if not self.to_batch:
            response = response[0]
        return response
...
class BLIPModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, half_precision=viper_config.blip_half_precision,
                 blip_v2_model_type=viper_config.blip_v2_model_type):
        super().__init__(gpu_number)
        ...
        with warnings.catch_warnings(), HiddenPrints("BLIP"), torch.cuda.device(self.dev):
            ...
            try:
                ...
                self.model = Blip2ForConditionalGeneration.from_pretrained(
                    f"Salesforce/{blip_v2_model_type}", quantization_config=bnb_config if half_precision else None,
...
class SaliencyModel(BaseModel):
    ...
    def __init__(self, gpu_number=0,
                 path_checkpoint=f'{viper_config.path_pretrained_models}/saliency_inspyrenet_plus_ultra'):
        ...
        with HiddenPrints("Saliency"):
            model = InSPyReNet(SwinB(pretrained=pretrained, path_pretrained_models=viper_config.path_pretrained_models),
...
class XVLMModel(BaseModel):
    ...
    def __init__(self, gpu_number=0,
                 path_checkpoint=f'{viper_config.path_pretrained_models}/xvlm/retrieval_mscoco_checkpoint_9.pth'):
    ...
    def forward(self, image, text, task='score', negative_categories=None):
        if isinstance(image, list):
            for i in range(len(image)):
                if image[i].shape[0] == 1:
                    image[i] = image[i].repeat(3, 1, 1)
